...
class FitnessFile:
    ...
    def validate_model(self, model, fileW, population, TrainX, TrainY, ValidateX,
        ...
        for i in range(numOfPop):
            ...
            q2_loo = DF.getTwoDecPoint(q2_loo)

            r2pred_validation = self.r2Pred(TrainY, ValidateY, Yhat_validation)
            r2pred_validation = DF.getTwoDecPoint(r2pred_validation)

            r2pred_test = self.r2Pred(TrainY, TestY, Yhat_test)
            r2pred_test = DF.getTwoDecPoint(r2pred_test)
            ...
            trackFitness[idx] = DF.getTwoDecPoint(fitness[i])

            trackModel[idx] = model_desc

            trackR2[idx] = DF.getTwoDecPoint(r2_train)
            trackQ2[idx] = DF.getTwoDecPoint(q2_loo)
            trackR2PredValidation[idx] = \
                DF.getTwoDecPoint(r2pred_validation)
            trackR2PredTest[idx] = DF.getTwoDecPoint(r2pred_test)
            ...
            for i in range(len(yHatTrain[idx])):
                yHatTrain[idx][i] = \
                    DF.getTwoDecPoint(yHatTrain[idx][i])

            yHatCV[idx] = Yhat_cv.tolist()
            for i in range(len(yHatCV[idx])):
                yHatCV[idx][i] = DF.getTwoDecPoint(yHatCV[idx][i])
            ...
            for i in range(len(yHatValidation[idx])):
                yHatValidation[idx][i] = \
                    DF.getTwoDecPoint(yHatValidation[idx][i])

            yTest[idx] = TestY.tolist()

            yHatTest[idx] = Yhat_test.tolist()
            for i in range(len(yHatTest[idx])):
                yHatTest[idx][i] = DF.getTwoDecPoint(yHatTest[idx][i])
